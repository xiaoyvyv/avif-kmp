...
function build() {
  ...
  echoRun mkdir -p src/jvmMain/resources/jni/$MACOS_ARCH/
  echoRun cmake -S src/jvmMain/ -B build/jni/$MACOS_ARCH/ \
    -DCMAKE_OSX_ARCHITECTURES=$CMAKE_ARCH
  echoRun cmake --build build/jni/$MACOS_ARCH/ --verbose
  echoRun cp -v build/jni/$MACOS_ARCH/libavif-jvm.* src/jvmMain/resources/jni/$MACOS_ARCH/
